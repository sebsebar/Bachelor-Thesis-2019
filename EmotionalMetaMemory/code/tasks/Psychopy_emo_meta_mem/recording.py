...
class Oximeter():
    ...
    Record 10s.
    >>> import serial
    >>> from PCP.recording import Oximeter
    ...
    >>> oximeter.setup()
    >>> oximeter.read(nSeconds=10)

    Plot recording
    >>> oximeter.plot(kind='rr')
    """
    def __init__(self, serial=None, sample=75):

        self.instant_rr = []
        self.lag = 0
        self.recording = []
        self.sample = sample
        self.dist = int(self.sample * 0.2)
        self.triggers = []
        self.times = []
        self.diff = []
        self.peaks = []
    ...
    def add_paquet(self, paquet, window=1):
        ...
        if not self.times:
            ...
            self.times.append(len(self.times)/self.sample)

        # Update threshold
        window = int(window * self.sample)
        ...
        #        (np.diff(np.where(self.triggers)[0])[-1]/self.sample)*1000)
        #else:
    ...
    def find_peaks(self):
        ...
        Returns
        -------
        Oximeter instance. The peaks timing is stored in the `peaks` attribute.
        """

        self.peaks = oxi_peaks(self.recording)

        # R-R intervals (in miliseconds)
        self.rr = (np.diff(self.peaks)/self.sample) * 1000
    ...
    def waitBeat(self):
        """Read Oximeter untila beat is detected.

        Notes
        -----
        Actually requiere 2 beats to be detected, and ensure that lag > 200 ms.
        """
        while True:
            if self.serial.inWaiting() >= 5:
                # Store Oxi level
