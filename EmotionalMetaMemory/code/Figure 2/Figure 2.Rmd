...
```{r Load and prepare PAVLOVIA data} 
...
Pav_data <- rename(Pav_data, c(Arousal=A_key_resp.keys, Valence=key_resp.keys,Word =word))
...
total <- rename(total, c(Valence_Mean_ANEW=ValMn, Arousal_Mean_ANEW=AroMn))
...
p <- insert_xaxis_grob(ANEW, xdensity2, grid::unit(.2, "null"), position = "top")
pp<- insert_yaxis_grob(p, ydensity2, grid::unit(.2, "null"), position = "right")
P1 <- ggdraw(pp)

ggsave(filename="Figure2_A.svg", plot=P1, height = 6, width = 6,dpi =600)
```
...
ggsave(filename="Figure2_B.svg", plot=P2, height = 6, width = 6,dpi =600)

```


```{r Plotting Figure C} 
P3 <- ggplot(total, aes(x=Valence_Mean_Pavlovia, y=Valence_Mean_ANEW))+ 
  geom_point(size = 1) + 
...
ggsave(filename="Figure2_C.svg", plot=P3, height = 6, width = 6,dpi =600)
```

```{r Plotting Figure D} 

# Plot rating consistency for arousal and valence
P4 <- ggplot(total, aes(x=Arousal_Mean_Pavlovia, y=Arousal_Mean_ANEW))+ 
  geom_point(size = 1)+ 
  geom_abline(intercept = 0, slope = 1,colour="red",size=1)+
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8,9), limits=c(1,9)) +
  scale_y_continuous(breaks = c(1,2,3,4,5,6,7,8,9), limits=c(1,9)) +
...
ggsave(filename="Figure2_D.svg", plot=P4, height = 6, width = 6,dpi =600)
