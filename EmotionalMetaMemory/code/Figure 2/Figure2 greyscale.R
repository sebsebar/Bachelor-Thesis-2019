...
PAV <- ggplot(total, aes(x = Valence_Mean_Pavlovia, y = Arousal_Mean_Pavlovia, colour=Val_Aro_ANEW)) + 
  geom_point(size = 1) + 
  scale_color_manual(values=c("#424242","#a4a4a4","#141414","#c8c8c8"))+ 
...
ggsave(filename="Figure2_B.svg", plot=P1, height = 6, width = 6,dpi =600)
...
ggsave(filename="Figure2_A.svg", plot=P2, height = 6, width = 6,dpi =600)
...
P3 <- ggplot(total, aes(x=Arousal_Mean_Pavlovia, y=Arousal_Mean_ANEW))+ 
  geom_point(size = 1)+ 
  geom_abline(intercept = 0, slope = 1,colour="#525252",size=1)+
  scale_x_continuous(breaks = c(1,2,3,4,5,6,7,8,9), limits=c(1,9)) +
  scale_y_continuous(breaks = c(1,2,3,4,5,6,7,8,9), limits=c(1,9)) +
  theme_bw()+
  labs(title = "D. Rating Consistency Arousal", x = "Mean Arousal Rating: Pavlovia", y = "Mean Arousal Rating: ANEW") +
  annotate("text", x = 3, y = 8.5, label = "Spearman Correlation",size=7.5, fontface="bold")+
  annotate("text", x = 3, y = 8, label = "rho = 0.7760279",size=7.5, fontface="bold") +
  annotate("text", x = 3, y = 7.5, label = "P-value = 2.2e-16",size=7.5, fontface="bold") + 
  theme(legend.position = "none")+ 
...
ggsave(filename="Figure2_D.svg", plot=P3, height = 6, width = 6,dpi =600)
...
P4 <- ggplot(total, aes(x=Valence_Mean_Pavlovia, y=Valence_Mean_ANEW))+ 
  ...
  annotate("text", x = 3, y = 8.5, label = "Spearman Correlation",size=7.5, fontface="bold")+
  annotate("text", x = 3, y = 8, label = "rho = 0.9324154",size=7.5, fontface="bold") +
  annotate("text", x = 3, y = 7.5, label = "P-value = 2.2e-16",size=7.5, fontface="bold") + 
  theme(legend.position = "none") + 
  theme(title = element_text(face = "bold", size = 14),axis.text.x = element_text(face = "bold", size = 14),axis.text.y = element_text(face = "bold", size = 14),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
    panel.background = element_blank(), axis.line = element_line(colour = "black"))

ggsave(filename="Figure2_C.svg", plot=P4, height = 6, width = 6,dpi =600)
