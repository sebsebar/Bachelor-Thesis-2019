...
"""
library(ggplot2)
ggplot(R_Model_Data,aes(x = Accuracy,y = Confidence, group=Arousal, color=Arousal)) + 
  geom_smooth(method=lm)+ facet_wrap(~Valence) + labs(title="Valence", x = "Accuracy", y = "Confidence") +theme_bw()+scale_x_continuous(breaks = c(0,1)) + theme(legend.position = "none") 
cet_null()#+ scale_color_manual(values=c("#2C5AA0","#AA0000")) 


ggplot(R_Model_Data,aes(x = Accuracy,y = Confidence, group=interaction(Valence), color=Valence)) + stat_summary(aes(colour=paste("mean",Valence),group=Valence), fun.y=mean, geom="line", size=1,position=position_dodge(0.15))+geom_point(aes(colour=paste("mean",Valence),group=Valence), stat = "summary", fun.y=mean,position=position_dodge(0.15),size=3) + labs(title="Arousal", x = "Accuracy", y = "Confidence") +theme_bw()+scale_x_continuous(breaks = c(30,60,90)) + theme(legend.position = "none") + geom_errorbar(aes(colour=paste("mean",Valence),group=Valence),stat = "summary", fun.data = mean_se, width = 0.3,position=position_dodge(0.15),size=1)+ theme(plot.title = element_text(size=25, face="bold.italic"),aspect.ratio=1,axis.title.x = element_text(size=20, face="bold"),axis.text.x = element_text(face = "bold", size = 12),axis.text.y = element_text(face = "bold", size = 12),axis.title.y = element_text(size=20, face="bold"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) + facet_wrap(~Arousal) +scale_x_continuous(breaks = c(0,1))+ scale_color_manual(values=c("#2C5AA0","#AA0000")) 

ggplot(R_Model_Data,aes(x = Accuracy,y = Confidence, col=SUBJ)) +stat_summary(aes(colour=paste("mean",Valence),group=Valence), fun.y=mean, geom="line", size=1,position=position_dodge(0.15))+geom_point(aes(colour=paste("mean",Valence),group=Valence), stat = "summary", fun.y=mean,position=position_dodge(0.15),size=3)+ labs(title="Arousal", x = "Accuracy", y = "Confidence") +theme_bw()+scale_x_continuous(breaks = c(30,60,90)) + theme(legend.position = "none") + geom_errorbar(aes(colour=paste("mean",Valence),group=Valence),stat = "summary", fun.data = mean_se, width = 0.3,position=position_dodge(0.15),size=1)+ theme(plot.title = element_text(size=25, face="bold.italic"),aspect.ratio=1,axis.title.x = element_text(size=20, face="bold"),axis.text.x = element_text(face = "bold", size = 12),axis.text.y = element_text(face = "bold", size = 12),axis.title.y = element_text(size=20, face="bold"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) + facet_wrap(~Arousal) +scale_x_continuous(breaks = c(0,1)) + geom_line(stat="smooth",method = "lm", alpha = 0.2)
```
